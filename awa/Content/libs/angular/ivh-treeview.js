  ...
  return {
    ...
    template: [
      '<input type="checkbox"',
        ...
        'ng-model="isSelected"',
        'ng-click="$event.stopPropagation(); resolveIndeterminateClick();"',
...
angular.module('ivh.treeview').directive('ivhTreeview', ['ivhTreeviewMgr', function(ivhTreeviewMgr) {
  ...
  return {
    ...
    controller: ['$scope', '$element', '$attrs', '$transclude', 'ivhTreeviewOptions', 'filterFilter', function($scope, $element, $attrs, $transclude, ivhTreeviewOptions, filterFilter) {
      ...
      // Merge any locally set options with those registered with the
      ...
      //---------------- CUSTOM FUNCTIONALITY - ANGEL-------------
      trvw.pick = function (node, isSelected, e){
        if (e.keyCode == 13 || e.which == 13 || e.keyCode == 32 || e.which == 32 || node.editme){ //if we get in here, not likely what we wanted
          return false;
        } else {
            var selected = $scope.$parent.selected;
            if (isSelected){
              selected.push(node)
            } else if (node.mSelected){//verify node is selected
            ...
            node.mSelected = isSelected;
        }

        //trvw.onCbChange(node, isSelected);
      }

      trvw.edit = function(node, e){
        //console.log('selected for edit:', $scope.$parent.selected)
        if(node){
          node.editme = undefined;
        } else {
          $scope.$parent.cancer.editType = 'none';
        }
      ...
      trvw.expand = function(node, isExpanded) {
        ...
        ivhTreeviewMgr.expand($scope.root, node, localOpts, isExpanded, deselect);

        function deselect(n){ //per requirements
          n.mSelected = false;
          if(scope.$parent.cancer) {scope.$parent.cancer.editType = 'none'}
          var selected = scope.$parent.selected;
          var ix = selected.indexOf(n);
          selected.splice(ix, 1);
        }
      };
...
angular.module('ivh.treeview').factory('ivhTreeviewBfs', ['ivhTreeviewOptions', function(ivhTreeviewOptions) {
  ...
  return function(tree, opts, cb) {
    ...
    while(next) {
      node = next[0];
      parents = next[1];
      // cb might return `undefined` so we have to actually check for equality against `false`
...
angular.module('ivh.treeview')
  .factory('ivhTreeviewMgr', ['ivhTreeviewOptions', 'ivhTreeviewBfs', function(ivhTreeviewOptions, ivhTreeviewBfs) {
    ...
    /* Angel: extensional utlities go here */ 
    //----------------------------------------------------------------------------------
    exports.stopEdit = function (tree, opts){
      ivhTreeviewBfs(tree, opts, function(n, p) {
        if(n.editme) {
          n.editme = undefined;
        }
     ...
     * @return {Object} Returns the ivhTreeviewMgr instance for chaining
     */
    exports.expand = function(tree, node, opts, isExpanded, deselect) {

      if(!isExpanded && typeof isExpanded === 'boolean' ){ //if collapsed deselect, per requirements
        var opt = {childrenAttribute: "items"}
        ivhTreeviewBfs(node,opt,function(n){
          deselect(n)
        })
      }
      if(arguments.length > 2) {
